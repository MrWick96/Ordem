...
//        exit();
    ...
    public function add() {
        ...
        if ($cliente_tipo == 1) {
            $this->form_validation->set_rules('cliente_cpf', '', 'trim|required|exact_length[14]|is_unique[clientes.cliente_cpf_cnpj]|callback_valida_cpf'); //18 char
        } else {
            $this->form_validation->set_rules('cliente_cnpj', '', 'trim|required|exact_length[18]|is_unique[clientes.cliente_cpf_cnpj]|callback_valida_cnpj'); //18 char
        }
...
//                exit();
    ...
    public function edit($cliente_id = NULL) {
        ...
        if (!$cliente_id || !$this->core_model->get_by_id('clientes', array('cliente_id' => $cliente_id))) {
            ...
            if ($cliente_tipo == 1) {
                $this->form_validation->set_rules('cliente_cpf', '', 'trim|required|exact_length[14]|callback_valida_cpf'); //18 char
            } else {
                $this->form_validation->set_rules('cliente_cnpj', '', 'trim|required|exact_length[18]|callback_valida_cnpj'); //18 char
            }
...
//                exit();
    ...
    public function del($cliente_id = NULL) {
        ...
        if(!$cliente_id || !$this->core_model->get_by_id('clientes', array('cliente_id' => $cliente_id))){
            ...
            $this->core_model->delete('cliente', array('cliente_id' => $cliente_id));
            redirect('clientes');
        }
